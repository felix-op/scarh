...
import { Bell, CheckCheck, CircleAlert, Clock3, Info, CheckCircle2, AlertTriangle } from "lucide-react";
...
export type NotificacionItem = {
	...
	descripcion: string;
	fecha: string;
	variante?: "info" | "error" | "exito" | "alerta";
...
type VentanaNotificacionesProps = {
	...
	onMarcarTodasLeidas?: () => void | Promise<void>;
};
...
export default function VentanaNotificaciones({
	...
	const cantidadNuevas = items.filter((item) => (item.estado ?? "nuevo") === "nuevo").length;
				...
				{items.length > 0 && (
					<div className="max-h-[65vh] space-y-3 overflow-y-auto px-6 py-5">
						{items.map((notificacion) => {
							...
							const TipoIcon = varianteIcono[notificacion.variante ?? "info"];
									...
									<div className="flex items-start justify-between gap-3">
										<div className="min-w-0 space-y-2">
											<div className="flex items-center gap-2">
												<span className="inline-flex items-center justify-center rounded-full bg-neutral-100 p-1.5 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-200">
