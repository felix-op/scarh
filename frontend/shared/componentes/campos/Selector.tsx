...
import { SelectProps } from "types/campos";
...
const Selector = forwardRef<HTMLSelectElement, SelectProps & SelectorProps>(
	(
		{
			children,
			error = false,
		...
		return (
			<div className="relative flex items-center" style={containerStyle}>
				<select
					{...rest}
					className={`
					w-full p-3 px-4 rounded-lg border-2 outline-none  appearance-none transition-colors
