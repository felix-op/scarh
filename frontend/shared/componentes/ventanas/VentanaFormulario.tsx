	...
	return (
		<>
			<Drawer open={open} onClose={handleClose} direction="right" dismissible={!edited}>
				<DrawerContent className="bg-transparent border-none py-4 pr-2">
					<div className="flex flex-col bg-ventana dark:border w-full h-full rounded-lg">
						<DrawerTitle className="flex justify-between items-center p-5">
						...
						<Formulario className="flex flex-col grow" valoresIniciales={valoresIniciales} onSubmit={onSubmit} onDirty={onDirty}>
							<DrawerDescription>{descripcion}</DrawerDescription>
							<div className="p-5 grow">
								{children}
							</div>
							<hr className="h-[2px] bg-ventana-secondary" />
							<DrawerFooter className="sm:flex-row justify-between p-5">
